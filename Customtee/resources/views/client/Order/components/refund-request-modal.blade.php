            ...
            <div class="modal-header border-0 pb-0 pt-4 px-4">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div>
                        ...
                        <div class="fs-8 text-muted">Đơn hàng #{{ $donHang->ma_don_hang }}</div>
                    </div>
                </div>
                ...
                <div class="modal-body px-4 py-3">
                                ...
                                @foreach ($donHang->chiTietDonHangs as $chiTiet)
                                    <div class="p-3 rounded-3 border border-slate-200 bg-white d-flex align-items-center justify-content-between flex-wrap gap-3">
                                        <div class="d-flex align-items-center gap-3">
                                            ...
                                            <div>
                                                <div class="fw-bold text-slate-900 fs-8">{{ $chiTiet->sanPham->ten_san_pham }}</div>
                                                @if ($chiTiet->bienThe)
                                                    <div class="fs-8 text-muted">
                                                        Phân loại: {{ $chiTiet->bienThe->color->ten_mau ?? '—' }} / {{ $chiTiet->bienThe->size->ten_kich_thuoc ?? '—' }}
                                                    </div>
                                                @endif
                                                <div class="fs-8 text-slate-500 mt-0.5">
                                                    Đã mua: <strong>{{ $chiTiet->so_luong }}</strong> × {{ number_format($chiTiet->don_gia, 0, ',', '.') }} ₫
                                                </div>
                                            </div>
                                        </div>
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="form-check">
                                                ...
                                                <label class="form-check-label fs-8 fw-medium" for="chiTiet{{ $chiTiet->id }}">Chọn trả</label>
                                            </div>
                            ...
                            <div class="d-flex flex-column gap-2">
                                @foreach ($donHang->chiTietDonHangs as $chiTiet)
                                    <div class="p-2-5 rounded-3 border border-slate-200 bg-slate-50 d-flex align-items-center justify-content-between gap-3">
                                        <div class="d-flex align-items-center gap-3">
                                            <img src="{{ $chiTiet->sanPham->hinh_anh_chinh ? asset('storage/' . $chiTiet->sanPham->hinh_anh_chinh) : 'https://via.placeholder.com/48' }}"
                                                alt="" class="rounded-2 border border-slate-200 object-fit-cover" width="48" height="48">
                                            <div>
                                                <div class="fw-bold text-slate-900 fs-8">{{ $chiTiet->sanPham->ten_san_pham }}</div>
                                                @if ($chiTiet->bienThe)
                                                    <div class="fs-8 text-muted">
                                                        {{ $chiTiet->bienThe->color->ten_mau ?? '—' }} / {{ $chiTiet->bienThe->size->ten_kich_thuoc ?? '—' }}
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="fs-8 text-muted">
                                            Số lượng: <strong class="text-slate-800">{{ $chiTiet->so_luong }}</strong>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    @else
                        <div class="mb-4">
                            ...
                            <textarea name="ly_do" id="ly_do" class="form-control rounded-3 fs-8" rows="3" required
                                placeholder="Vui lòng mô tả chi tiết lý do (sản phẩm lỗi, rách chỉ, không đúng size/màu, sai mô tả...)">{{ old('ly_do') }}</textarea>
                            @if (isset($errors) && $errors->has('ly_do'))
                                <div class="text-danger small mt-1">{{ $errors->first('ly_do') }}</div>
                            @endif
                        </div>
                    @endif
                    ...
                    @if (!$isOnlineCancelRefund)
                        <div class="mb-4">
                            ...
                            @if (isset($errors) && $errors->has('hinh_anh.*'))
                                <div class="text-danger small mt-1">{{ $errors->first('hinh_anh.*') }}</div>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="mb-3">
                        ...
                        <div id="upload_section">
                            ...
                            @if (isset($errors) && $errors->has('hinh_tai_khoan.*'))
                                <div class="text-danger small mt-1">{{ $errors->first('hinh_tai_khoan.*') }}</div>
                            @endif
                        </div>
