...
<div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden order-status-card">
    <div class="card-header bg-white border-bottom border-slate-100 py-3 px-4 d-flex justify-content-between align-items-center">
        ...
        <div class="fs-13 text-muted">
            <?php if($effectiveTrangThai === 'da_hoan_thanh'): ?>
            ...
            <?php elseif($effectiveTrangThai === 'da_huy'): ?>
    ...
    <div class="card-body p-4">
        <?php if($effectiveTrangThai === 'da_huy'): ?>
            ...
            <div class="p-3-5 rounded-3 bg-rose-50/80 border border-rose-200">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <?php if($donHang->ly_do_huy_boi_admin): ?>
                            <div class="p-2-5 rounded-3 bg-white border border-rose-200 fs-13 text-slate-800 mt-2-5">
                                ...
                                <span><?php echo e($donHang->ly_do_huy_boi_admin); ?></span>
                            </div>
                        <?php elseif($donHang->ly_do_yeu_cau_huy): ?>
                            <div class="p-2-5 rounded-3 bg-white border border-rose-200 fs-13 text-slate-800 mt-2-5">
                                <strong class="text-rose-800"><i class="bi bi-info-circle me-1"></i> Lý do hủy của bạn:</strong>
                                <span><?php echo e($donHang->ly_do_yeu_cau_huy); ?></span>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
        ...
        <?php elseif($isPendingCancelRequest): ?>
        ...
        <?php else: ?>
            ...
            <div class="timeline-stepper py-2">
                <div class="stepper-wrapper">
                    ...
                    <?php $__currentLoopData = $steps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $step): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        ?>
                        <div class="stepper-step <?php echo e($isCompleted ? 'step-completed' : ''); ?> <?php echo e($isCurrent ? 'step-current' : ''); ?> <?php echo e($isPending ? 'step-pending' : ''); ?>">
                            ...
                            <div class="step-content text-center mt-2">
                                <div class="step-title fw-semibold fs-13"><?php echo e($step['label']); ?></div>
                                <div class="step-subtitle text-muted fs-12"><?php echo e($step['sub']); ?></div>
                            </div>
                        </div>
                        <?php $i++; ?>
            ...
            <?php if($donHang->da_giao_at || $donHang->da_nhan_hang_at): ?>
                <div class="row g-2 mt-4 pt-3 border-top border-slate-100">
                    <?php if($donHang->da_giao_at): ?>
                        <div class="col-sm-6">
                            <div class="d-flex align-items-center gap-2 p-2-5 rounded-3 bg-slate-50 border border-slate-200/60 fs-13">
                                ...
                                <strong class="text-slate-800"><?php echo e($donHang->da_giao_at->format('H:i - d/m/Y')); ?></strong>
                            </div>
                        </div>
                    <?php endif; ?>
                    <?php if($donHang->da_nhan_hang_at): ?>
                        <div class="col-sm-6">
                            <div class="d-flex align-items-center gap-2 p-2-5 rounded-3 bg-slate-50 border border-slate-200/60 fs-13">
                                <i class="bi bi-box-seam text-emerald-600"></i>
                                <span class="text-slate-600">Thời gian nhận hàng:</span>
                                <strong class="text-slate-800"><?php echo e($donHang->da_nhan_hang_at->format('H:i - d/m/Y')); ?></strong>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <?php if($donHang->ly_do_tu_choi_huy && in_array($donHang->trang_thai, ['dang_xu_ly', 'dang_giao'], true)): ?>
                <div class="alert alert-warning border border-warning-subtle rounded-3 p-3 mt-3 mb-0 fs-13">
                    ...
                    <span><?php echo e($donHang->ly_do_tu_choi_huy); ?></span>
                </div>
            <?php endif; ?>
        <?php endif; ?>

        
        <?php if($yeuCauHoanTien && $currentRefund): ?>
            <div class="mt-4 card border border-<?php echo e($currentRefund[1]); ?>-subtle shadow-2xs rounded-3 overflow-hidden">
                <div class="card-header bg-<?php echo e($currentRefund[1]); ?>-subtle text-<?php echo e($currentRefund[1]); ?>-emphasis fw-semibold py-2 px-3 d-flex justify-content-between align-items-center fs-8">
                    <span class="d-inline-flex align-items-center gap-2">
                        <i class="bi <?php echo e($currentRefund[2]); ?>"></i> Yêu cầu Hoàn tiền / Trả hàng
                    </span>
                    <span class="badge bg-<?php echo e($currentRefund[1]); ?> rounded-pill"><?php echo e($currentRefund[0]); ?></span>
                </div>
                <div class="card-body p-3 fs-8">
                    <div class="row g-2">
                        <div class="col-sm-6 d-flex justify-content-between py-1 border-bottom border-slate-100">
                            ...
                            <strong class="text-slate-900"><?php echo e(number_format($yeuCauHoanTien->so_tien_yeu_cau ?? 0, 0, ',', '.')); ?> ₫</strong>
                        </div>
                        <?php if($yeuCauHoanTien->created_at): ?>
                            <div class="col-sm-6 d-flex justify-content-between py-1 border-bottom border-slate-100">
                                <span class="text-muted">Thời gian gửi yêu cầu:</span>
                                <strong><?php echo e($yeuCauHoanTien->created_at->format('H:i - d/m/Y')); ?></strong>
                            </div>
                        <?php endif; ?>
                    </div>
                    <?php if($yeuCauHoanTien->ly_do): ?>
                        <div class="mt-2 text-slate-700">
                            <span class="text-muted">Lý do:</span> <?php echo e($yeuCauHoanTien->ly_do); ?>

                        </div>
                    <?php endif; ?>

                    <?php if($yeuCauHoanTien->trang_thai === 'da_hoan_tien' && $yeuCauHoanTien->hinh_anh_xac_nhan): ?>
                        <div class="mt-3">
                            <button type="button" class="btn btn-outline-success btn-sm rounded-pill d-inline-flex align-items-center gap-1-5 px-3 py-1 fs-8"
                                data-bs-toggle="modal" data-bs-target="#refundProofModal<?php echo e($yeuCauHoanTien->id); ?>">
                                <i class="bi bi-file-earmark-image"></i>
                                <span>Xem ảnh minh chứng chuyển khoản</span>
                            </button>
                        </div>

                        <div class="modal fade" id="refundProofModal<?php echo e($yeuCauHoanTien->id); ?>" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                <div class="modal-content rounded-4 border-0 shadow">
                                    ...
                                    <div class="modal-body text-center p-4">
                                        <a href="<?php echo e(asset('storage/' . $yeuCauHoanTien->hinh_anh_xac_nhan)); ?>" target="_blank" rel="noopener">
                                            <img src="<?php echo e(asset('storage/' . $yeuCauHoanTien->hinh_anh_xac_nhan)); ?>" alt="Ảnh minh chứng hoàn tiền" class="img-fluid rounded-3 shadow-sm" style="max-height: 500px;">
                                        </a>
                    ...
                    <?php else: ?>
                        <p class="text-muted mt-2 mb-0 fs-8"><?php echo e($currentRefund[3]); ?></p>
                    <?php endif; ?>
                </div>
            </div>
        <?php endif; ?>
    ...
    <div class="modal fade" id="modalHuyDon" tabindex="-1" aria-labelledby="modalHuyDonLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-4 border-0 shadow-lg">
                <form action="<?php echo e(route('order.cancel', $donHang->id)); ?>" method="post">
                    <?php echo csrf_field(); ?>
                    <div class="modal-header border-0 pb-0 pt-4 px-4">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <h5 class="modal-title fw-bold fs-5 text-slate-900" id="modalHuyDonLabel">
                                <?php echo e($laYeuCauHuyOnline ? 'Yêu cầu hủy đơn hàng' : 'Xác nhận hủy đơn hàng'); ?>
                    ...
                    <div class="modal-body px-4 py-3">
                        ...
                        <?php else: ?>
                            <p class="text-muted fs-8 mb-3">
                                Bạn có chắc chắn muốn hủy đơn hàng <strong>#<?php echo e($donHang->ma_don_hang); ?></strong>? Thao tác này không thể hoàn tác.
                            </p>
                        <?php endif; ?>
                        ...
                        <?php if(!$boQuaLyDoHuy): ?>
                            <div class="mb-2">
                                ...
                                <textarea name="ly_do_yeu_cau_huy" id="ly_do_yeu_cau_huy" class="form-control rounded-3 fs-8"
                                    rows="3" required maxlength="2000" placeholder="Vui lòng cho chúng tôi biết lý do hủy đơn của bạn..."><?php echo e(old('ly_do_yeu_cau_huy')); ?></textarea>
                                <?php $__errorArgs = ['ly_do_yeu_cau_huy'];
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div class="modal-footer border-0 pt-0 px-4 pb-4">
                        ...
                        <button type="submit" class="btn btn-danger rounded-pill px-4 py-2 fs-8 fw-semibold">
                            <?php echo e($laYeuCauHuyOnline ? 'Gửi yêu cầu hủy' : 'Xác nhận hủy'); ?>
    ...
    <?php if(isset($errors) && $errors->has('ly_do_yeu_cau_huy')): ?>
        <script>
    ...
    <?php endif; ?>
<?php endif; ?>
<?php /**PATH D:\e7\laragon\www\DATN\DATN-CustomTee\Customtee\resources\views\client\Order\components\order-status.blade.php ENDPATH**/ ?>