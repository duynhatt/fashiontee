<nav aria-label="breadcrumb" class="mb-2">
    <ol class="breadcrumb bg-transparent p-0 m-0 fs-13">
        <li class="breadcrumb-item">
            <a href="<?php echo e(route('order')); ?>" class="text-decoration-none text-muted d-inline-flex align-items-center gap-1 hover-text-primary">
                <i class="bi bi-bag-check"></i>
                <span>Đơn hàng của tôi</span>
            </a>
        </li>
        <li class="breadcrumb-item active text-slate-700 fw-medium" aria-current="page">
            Chi tiết #<?php echo e($donHang->ma_don_hang); ?>
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 pb-3 mb-4 border-bottom border-slate-200">
    <div>
        <div class="d-flex flex-wrap align-items-center gap-2 mb-1-5">
            <h1 class="fs-20 fw-bold text-slate-900 mb-0">Đơn hàng #<?php echo e($donHang->ma_don_hang); ?></h1>
            <span class="order-status-pill <?php echo e($currentBadge['class']); ?> d-inline-flex align-items-center gap-1-5 px-3 py-1 rounded-pill fw-semibold fs-12 border">
                <i class="bi <?php echo e($currentBadge['icon']); ?>"></i>
                <span><?php echo e($currentBadge['label']); ?></span>
            </span>
        </div>
        <div class="d-flex flex-wrap align-items-center gap-2 text-slate-500 fs-13">
            <span class="d-inline-flex align-items-center gap-1">
                <i class="bi bi-calendar3"></i>
                <span>Đặt lúc <?php echo e($donHang->created_at->format('H:i - d/m/Y')); ?></span>
            </span>
            <?php if($donHang->chiTietDonHangs): ?>
                <span class="text-slate-300">•</span>
                <span><?php echo e($donHang->chiTietDonHangs->sum('so_luong')); ?> món hàng</span>
            <?php endif; ?>
        </div>
        <?php if($canShowReturnCountdown): ?>
            <div class="mt-2">
                <div class="d-inline-flex align-items-center gap-2 px-3 py-1-5 rounded-pill bg-amber-50 text-amber-800 border border-amber-200 fs-12 fw-medium"
                     id="returnCountdown" data-deadline-ts="<?php echo e($returnDeadlineTs); ?>">
    ...
    <div class="d-flex align-items-center gap-2">
        <a href="<?php echo e(route('order')); ?>" class="btn btn-outline-secondary rounded-pill px-3 py-1-5 fs-13 fw-medium d-inline-flex align-items-center gap-1-5 text-slate-700 bg-white shadow-2xs">
...
<?php endif; ?>
<?php /**PATH D:\e7\laragon\www\DATN\DATN-CustomTee\Customtee\resources\views\client\Order\components\order-header.blade.php ENDPATH**/ ?>